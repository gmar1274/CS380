public class Hasher {
    public static byte[] hash(byte[] input) {
        if(input.length<1) input = new byte[]{0};
    ...
    private static byte[][] organizeBytesInto4(byte[] bytes) {
        int l = (int)Math.ceil(bytes.length / 4.0);
        byte[][] result = new byte[l][4];
        
        for(int i=0; i<l*4; i++) {
            byte b = i<bytes.length ? bytes[i] : 0;
            result[i/4][i%4] = b;
        }
    ...
    private static String byteMatrixToString(byte[][] bytes) {
        if(bytes==null) return null;
        if(bytes.length<1) return "";
        StringBuilder sb = new StringBuilder();
        ...
        for(int r=0; r<bytes.length; r++) {
            for (int c=0; c<bytes[r].length; c++)
                sb.append(String.format("%02X ", bytes[r][c]));
